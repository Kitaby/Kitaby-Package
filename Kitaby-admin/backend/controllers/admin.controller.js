...
const getBibInfo = async (req, res) => {
    try {
        ...
        const bookNames = await Promise.all(bib.avalaibleBooks.map(async (book) => {
            const foo = await Book.findOne({ isbn: book.isbn });
            return foo.title ? foo.title: foo.name;
        }))
        res.status(200).send({ ...bib._doc, avalaibleBooks: bookNames });
